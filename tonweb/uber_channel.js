...
const init = async () => {
    ...
    if(typeof process.env.SEED_A === 'undefined' || typeof process.env.SEED_B === 'undefined'){
        throw new Error("Please provive .env file with SEED_A and SEED_B")
    }
    
    
    const customSeedA = process.env.SEED_A
    const customSeedB = process.env.SEED_B

    const keyPairA = tonweb.utils.keyPairFromSeed(
        tonweb.utils.base64ToBytes(customSeedA)); // Obtain key pair (public key and private key)
    const keyPairB = tonweb.utils.keyPairFromSeed(
        tonweb.utils.base64ToBytes(customSeedB)); // Obtain key pair (public key and private key)
